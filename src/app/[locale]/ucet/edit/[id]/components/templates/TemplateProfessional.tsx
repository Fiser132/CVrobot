const TemplateProfessional = ({ cvData, photoPreview }: any) => (
  <div className="bg-gray-100 p-8 rounded-lg shadow-inner text-black space-y-6">
    <div className="flex items-center gap-6">
      {photoPreview && (
        <img src={photoPreview} alt="Foto" className="w-24 h-24 rounded-full object-cover border-2" />
      )}
      <div className="bg-secondary text-white">
        <h1 className="text-3xl font-bold">{cvData.firstName} {cvData.lastName}</h1>
        <p className="text-sm">{cvData.email} | {cvData.phone}</p>
        <p className="text-sm">{cvData.street}, {cvData.city}, {cvData.postalCode}, {cvData.country}</p>
      ...
      <section>
        ...
        {cvData.education.map((edu: any, i: number) => (
          <div key={i} className="text-sm mt-2">
            ...
            <p>{edu.field} ({edu.startYear}–{edu.endYear})</p>
          </div>
        ))}
      </section>
    )}
    ...
    {cvData.certificates?.length > 0 && (
      <section>
        ...
        {cvData.certificates.map((cert: any, i: number) => (
          <p key={i} className="text-sm">{cert.name} – {cert.description}</p>
        ))}
      </section>
    )}
